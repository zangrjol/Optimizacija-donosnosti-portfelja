...
def getObcina(x):
    c = baza.cursor()
    c.execute("SELECT id FROM obcina WHERE ime = ?", [x]) #namesto vprašaja bo napisano, kar boš dal v seznam [x]
    r = c.fetchone() #vrne eno vrstico, fetchall() vrne vse vrstice
    id = -1
    if r == None:
...
##    with open('deset.csv', 'r') as podatki
##        t = csv.reader(podatki, delimiter=';', quotechar='|')
##        for vrstica in t:
##            print(t)
    ...
    for i, l in enumerate(raz):
        ...
        # Če je vrstica prazna, naslednja določa spol
            ...
            id = getObcina(l[0]) #l[0] nam da ime obcine (poglej v CSV

            for j in range(1,len(l)):
                baza.execute("REPLACE INTO podatki VALUES (?,?,?,?,?,?)",
                             [id,leto[j],polletje[j],spol,starost[j],int(l[j])])
#REPLACE je enako kot INSERT, le da če vrstica že obstaja jo zamenja
   
    # Obvezno moramo narediti commit, sicer se ne zgodi nič
    baza.commit()  #sedaj se sele preveri ali konsistentno
    #ce bi delali commit po vsakem for-u bi bilo počasi in tudi če bi se vsem zgodila napaka, bi dobili na pol uvožene podatki
    print("Končano.")
    
# POIZVEDUJEMO ZA PODATKI
def query(obcina=None, spol=None, minst=None, maxst=None):
    c = baza.cursor()
    if obcina != None: id = getObcina(obcina)
    c.execute("SELECT leto, polletje, sum(stevilo) FROM podatki WHERE obcina = ? AND spol = ? AND starost <= ? AND starost <= ? GROUP BY leto,polletje",[id,getSpol(spol),minst,maxst])
    mat = c.fetchall()
    c.close()
    return(mat)
...
#Da pozenes program napises v glavno konzolo: importData()
