...
def query(obcina=None, spol=None, minst=None, maxst=None):
    ...
    c.execute("SELECT leto, polletje, sum(stevilo) FROM podatki WHERE obcina = ? AND spol = ? AND starost >= ? AND starost <= ? GROUP BY leto, polletje",
              [id, getSpol(spol), minst, maxst])
    vas = c.fetchall()
    c.close()
    return(vas)
